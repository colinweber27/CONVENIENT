...
# Date: 9 August 2023
# Purpose: To convert a line of text listing some target composition in GENIE format to the equivalent in NuWro format.
# GENIE format is PDG[fraction],..., where PDG is the PDG code of a given nucleus in the target, and fraction is the fraction of the total that is made up of that nuclus.
# NuWro format is [# protons] [# neutrons] [# nuclei] x ..., where # protons is the number of protons in a given nucleus in the target, # neutrons is the number of neutrons in that same nucleus, and # nuclei is the number of that nucleus in the target.

# Command: bash convert_genie_comps_to_nuwro.sh -i input file -p 4 -o output file name
# Note that all options must be set. The option -p is for the GENIE precision, which we define to be the number of decimal places used in the fractions for the GENIE target composition.
...
do
	...
	nucleons=$((10#$nucleons)) # Strip leading 0's
