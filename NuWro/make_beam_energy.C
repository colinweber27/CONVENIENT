...
void make_beam_energy(std::string flux_file_str, std::string flux_histo_str)
{
	// Read in the flux file
	TFile* flux_file = new TFile(flux_file_str.c_str());

	// Read in the energy spectrum from the flux file
	TH1D* flux_histo = (TH1D*)flux_file->Get(flux_histo_str.c_str());
	...
	std::string full_beam_energy_path = cwd + "/" + \
		"NuWro/beam_energies/" + flux_file_str + ".txt";
