...
void SampleBreakdown(const bool isRHC = true, const bool makeSpecs = true, const bool makePlots = true){
  ...
  if(makeSpecs){
    ...
    std::string filename = "rhc_true_energy_breakdown_hists_tot.root";
  ...
  if(makePlots){
    ...
    TFile inF("rhc_true_energy_breakdown_hists_tot.root");
	...
	TFile* nuebar_file = new TFile("rhc_nuebar_histos.root", "RECREATE");
	...
	TFile* nue_file = new TFile("rhc_nue_histos.root", "RECREATE");
    ...
    iNumuTot->GetYaxis()->SetTitle("10^{3} Events / 11.0 #times 10^{20} POT");
	...
	TFile* numu_file = new TFile("rhc_numu_histos.root", "RECREATE");
    ...
    iAntiNumuTot->GetYaxis()->SetTitle("10^{5} Events / 11.0 #times 10^{20} POT");
    ...
    TFile* numubar_file = new TFile("rhc_numubar_histos.root", "RECREATE");
