...
# doc-5348
void_volume_fraction = 0.268

rock_volume_fraction = 1 - void_volume_fraction

# Should be mostly dry since it's a triangle shape above grade, right?
# So 25%, to pick a semi-random reasonable number. But add 13% to
# roughly account for snow cover as well without having to explicitly
# simulate snow on the berm. (Mean 18" of snow over the year, divided by
# 2 to make it water, as a fraction of mean berm height.)
...
def fdbermrock_composition():
  return make_composition("fdbermrock", fractions, compositions)
